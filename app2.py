...
def is_blocked_domain(url): #checks if a URL belongs to the blocked domains database
    conn = get_db_connection(BLOCKED_DOMAINS_DB)
    domain = urlparse(url).netloc

    # Check if the domain or any of its subdomains are blocked
    blocked = conn.execute('SELECT 1 FROM blocked_domains WHERE ? LIKE CONCAT(domain, \'.%\') OR domain = ?', (domain, domain)).fetchone()
    conn.close()
